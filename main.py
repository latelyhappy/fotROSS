...
<head>
    <meta charset="UTF-8">
    <title>ROSS Sniper V8.5 - 終極精準版</title>
    <style>
        ...
        .row-ross { background: rgba(255, 51, 102, 0.2) !important; border-left: 3px solid #ff3366; }
        .row-hod { background: rgba(63, 185, 80, 0.1) !important; border-left: 3px solid #3fb950; }
        .row-support { background: rgba(88, 166, 255, 0.1) !important; border-left: 3px solid #58a6ff; }
...
<body>
    <div class="window" id="win-alerts" style="top:10px; left:10px; width:720px; height:450px;"><div class="title-bar"><span>🚨 即時動能警報 (V8.5 精準識別)</span></div><div class="content" id="alert-list"></div></div>
    <div class="window" id="win-gainers" style="top:470px; left:10px; width:720px; height:480px;"><div class="title-bar"><span>🏆 強勢榜 (1-30 USD)</span></div><div class="content" id="gainer-list"></div></div>
    <div class="window" id="win-quote" style="top:10px; left:740px; width:480px; height:450px;"><div class="title-bar"><span>📰 24H 即時情報翻譯</span></div><div class="content" id="news-list">點擊個股開始狙擊...</div></div>
    <div class="window" id="win-pillars" style="top:470px; left:740px; width:480px; height:480px;"><div class="title-bar"><span>📊 核心五檔數據</span></div><div class="content" id="pillar-list"></div></div>

    <div id="sys-status">🔄 雲端數據同步中...</div>
    ...
    <script>
        async function update() {
            try {
                ...
                document.getElementById('sys-status').innerText = '✅ V164.5 運行中 | ' + new Date().toLocaleTimeString();
                
                // 警報清單：增加「換手」與「跳空」欄位
                let alertH = '<div class="grid-row grid-th" style="grid-template-columns: 0.7fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 1.2fr;"><div>時間</div><div>代碼</div><div>價格</div><div>量比</div><div>換手</div><div>跳空</div><div>訊號</div></div>';
                data.alerts.forEach(a => {
                    let cls = a.Type.includes('Ross') ? 'row-ross' : (a.Type.includes('HOD') ? 'row-hod' : (a.Type.includes('支撐') ? 'row-support' : ''));
                    alertH += `<div class="grid-row ${cls}" style="grid-template-columns: 0.7fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 1.2fr;" onclick="loadDetail('${a.Code}')">
                ...
                data.stocks.forEach(s => {
                    gainerH += `<div class="grid-row" style="grid-template-columns: 1fr 1fr 1fr 1.2fr 1fr;" onclick="loadDetail('${s.Code}')">
                        <div class="text-blue">${s.Code}</div><div>${s.Price}</div><div class="text-green">${s.Change}</div><div>${s.Volume}</div><div>${s.RVOL}</div>
                    </div>`;
                });
        ...
        async function loadDetail(sym) {
            ...
            document.getElementById('pillar-list').innerHTML = `
                <div style="display:flex; flex-wrap:wrap; gap:5px;">
                    ...
                    <div class="p-box">EMA9 距離<div class="p-val">${d.EMA_Dist || '0.0%'}</div></div>
                </div>`;
        }
...
# --- [ 2. 核心運算引擎 - 包含數據預補全邏輯 ] ---
[cite_start]def format_volume_km(vol): # [cite: 1]
    if vol <= 0: return "N/A"
    if vol >= 1e6: return f"{vol/1e6:.1f}M"
    if vol >= 1e3: return f"{vol/1e3:.0f}K"
    return str(int(vol))

[cite_start]def parse_vol_str(v): # [cite: 1]
        ...
        try:
            ...
            if table:
                ...
                for tr in table.find_all('tr')[1:30]:
                    ...
                    if 1.0 <= p_num <= 30.0:
                        # 核心數據同步與預抓取
                        cell = MASTER_BRAIN["details"].get(sym, {"HOD_num": 0, "NewsList": [], "PM_High": 0})
                        f, a, prev = fetch_advanced_data(sym) if sym not in stock_info_cache else stock_info_cache[sym]
                        ...
                        turnover = f"{(vol_n/f*100):.1f}%" if f > 0 else "0.0%"
                        drop_val = ((p_num - cell['HOD_num']) / cell['HOD_num'] * 100) if cell['HOD_num'] > 0 else 0
                        rvol_val = vol_n / a if a > 0 else 1.0

                        # 警報判定邏輯
                        a_type = "🆕NEW"
                        if rvol_val > 10.0: a_type = "💥爆量噴發"
                        elif float(gap.replace('%','')) > 3.0 and rvol_val > 2.0: a_type = "⚡跳空起漲"
                        elif drop_val < -2.0: a_type = "🔴Ross警告" if p_num < (cell['HOD_num'] * 0.98) else "🔵震倉觀察"
                        elif p_num >= cell["HOD_num"] and rvol_val > 1.0: a_type = "🔥HOD突破"
                        ...
                        item = {
                            ...
                            "Time": datetime.now().strftime('%H:%M:%S'), "Gap": gap,
                            "FloatStr": format_volume_km(f), "Turnover": turnover, "Type": a_type,
...
if __name__ == '__main__':
    ...
    # [cite_start]解決雲端部署 502/404 問題 [cite: 1]
