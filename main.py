...
<head>
    <meta charset="UTF-8">
    <title>ROSS Sniper V165.5 - 全功能完整版</title>
    <style>
...
<body>
    ...
    <div class="window" id="win-rank" style="top:340px; left:500px; width:480px; height:600px;"><div class="title-bar">🏆 排行掃描 (全資料)</div><div class="content" id="rank-list"></div><div class="resize-handle"></div></div>
    <div class="window" id="win-details" style="top:670px; left:10px; width:480px; height:310px;"><div class="title-bar">📊 指標詳情與新聞翻譯</div><div class="content" id="detail-list">點擊代碼查看...</div><div class="resize-handle"></div></div>
...
def get_static(ticker): #
    ...
    try:
        t = yf.Ticker(ticker)
        info = t.info
        f = info.get('floatShares', 1000000)
        a = info.get('averageVolume', 500000)
        ...
        try:
            ...
            for tr in table.find_all('tr')[1:25]:
                ...
                if 1.0 <= p_num <= 30.0:
                    ...
                    if gap_p > 3.0 and rvol > 5.0: item["Type"] = "🚀 第一根狙擊"; temp_snip.append(item)
                    if drop_p < -2.0: item["Type"] = "🔴 Ross 下跌"; temp_drop.append(item)
                    if p_num >= cell["HOD"]: item["Type"] = "🔥 HOD 突破"
                    
                    if "掃描" not in item["Type"]: live_history.insert(0, item)
                    
                    # 非同步更新新聞，只針對新出現的標的或狙擊標的
                    if not cell["NewsList"] or "🚀" in item["Type"]:
                        cell["NewsList"] = fetch_news(sym)

                    temp_stocks.append(item)
                    MASTER_BRAIN["details"][sym] = {
                        "HOD": cell["HOD"], "NewsList": cell["NewsList"], "Gap": item["Gap"], 
                        "Turnover": item["Turnover"], "RVOL": item["RVOL"], "FloatStr": item["FloatStr"]
                    }

            live_history = live_history[:20]
            MASTER_BRAIN.update({"stocks": temp_stocks, "sniper": temp_snip, "drop": temp_drop, "live": live_history, "last_update": current_time})
